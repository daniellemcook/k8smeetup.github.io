...
集群运维人员 cluster-operator 可以使用服务目录 service-broker 浏览其提供的托管服务 managed-service 列表，提供托管服务实例并与之绑定，以使其可以被 Kubernetes 集群中的应用程序使用。
...
应用开发者 application-developer 希望使用消息队列作为其在 Kubernetes 集群中运行的应用程序的一部分。但是，它们不想承受建立这种服务的开销，也不想自行管理。幸运的是，有一家云服务提供商通过它们的 *服务代理* 将消息队列作为 *托管服务* 提供。


集群运维人员 cluster-operator 可以设置服务目录并使用它与云服务提供商的服务代理 service-broker 通信，以此提供消息队列服务的实例并使其对 Kubernetes 中的应用程序可用。因此，应用开发者" term_id="application-developer" %}可以不用关心消息队列的实现细节，也不用对其进行管理。它们的应用程序可以简单的将其作为服务使用。
...
集群运维人员 cluster-operator 可以使用服务目录 API 资源来提供托管服务并使其在 Kubernetes 集群内可用。涉及的步骤有：
...
首先，集群运维人员 cluster-operator 在 `servicecatalog.k8s.io` 组内创建一个 `ClusterServiceBroker` 资源。此资源包含访问服务代理终结点所需的 URL 和连接详细信息。
...
3. 然后集群运维人员" term_id="cluster-operator" %} 可以使用以下命令获取可用托管服务的列表：
...
集群运维人员 cluster-operator 可以通过创建一个 `ServiceInstance` 资源来启动一个新实例的配置。
...
3. 然后集群运维人员" term_id="cluster-operator" %}可以检查实例的状态是否就绪。


### 绑定到托管服务


在设置新实例之后， 集群运维人员 cluster-operator 必须绑定到托管服务才能获取应用程序使用服务所需的连接凭据和服务账户的详细信息。该操作通过创建一个 `ServiceBinding` 资源完成。
